...
@section('content-main')
    <div class="card">
        <div class="card-header">Chỉnh sửa cập nhật thông tin</div>
        <div class="card-body">
            <div class="row">
                <div class="col-10 mx-auto">
                    <form action="{{ route('faculty.update',['faculty'=>$faculty->id]) }}" method="post">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            @error('phone')
                                <div class="warning-danger"><i class="fas fa-exclamation-triangle"></i>{{ $message }}</div>
                            @enderror
                            <input type="text" name="phone" id="phone" value="{{ $faculty->phone }}"
                        ...
                        <div class="form-group">
                            ...
                            @error('email')
                                <div class="warning-danger"><i class="fas fa-exclamation-triangle"></i>{{ $message }}</div>
                            @enderror
                            <input type="text" name="email" id="phone" value="{{ $faculty->email }}"
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control input-control" name="dean_id" id="dean_id">
                            @foreach ($user as $us)
                                <option value="{{ $us->id }}" {{ ($us->id === $faculty->dean_id) ? 'selected' : "" }}>{{ $us->name }}</option>
                            @endforeach
                          </select>
                        </div>
